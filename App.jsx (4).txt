...
const TELC_LEVELS = {
  A1: {
    lesen: { label: "Lesen", teils: [
      { name: "Teil 1 — Zuordnen (moslashtirish)", type: "mcq" },
...
function makeEmptyTeilAnswers(cfg) {
  ...
  ["lesen", "sprachbausteine", "hören"].forEach((key) => {
    if (cfg[key]) result[key] = cfg[key].teils.map(() => []);
  });
  return result;
}
...
function CreateExam({ onCancel, onSave, saving }) {
  ...
  const updateTeil = (sectionKey, teilIdx, list) => {
    setTeilAnswers((prev) => ({ ...prev, [sectionKey]: prev[sectionKey].map((t, i) => (i === teilIdx ? list : t)) }));
  };
  const mk = (sectionKey, teilIdx) => {
    const list = teilAnswers[sectionKey][teilIdx];
    return {
      onAdd: () => updateTeil(sectionKey, teilIdx, [...list, emptyMCQ(cfg[sectionKey].teils[teilIdx].type)]),
      onUpdate: (id, patch) => updateTeil(sectionKey, teilIdx, list.map((q) => (q.id === id ? { ...q, ...patch } : q))),
      onRemove: (id) => updateTeil(sectionKey, teilIdx, list.filter((q) => q.id !== id)),
    };
  };

  const totalTestQuestions = ["lesen", "sprachbausteine", "hören"]
    .filter((k) => cfg[k])
    .reduce((sum, k) => sum + teilAnswers[k].reduce((s, t) => s + t.length, 0), 0);
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 18 }}>
        ...
        {step < steps.length - 1 ? (
          ...
          <Btn icon={Save} disabled={!canSave || saving} onClick={() => onSave({
            ...
            sections: {
              lesen: cfg.lesen ? { passage: lesenPassage.trim(), teils: cfg.lesen.teils.map((t, i) => ({ name: t.name, type: t.type, questions: teilAnswers.lesen[i].filter((q) => q.question.trim()) })) } : null,
              sprachbausteine: cfg.sprachbausteine ? { teils: cfg.sprachbausteine.teils.map((t, i) => ({ name: t.name, type: t.type, questions: teilAnswers.sprachbausteine[i].filter((q) => q.question.trim()) })) } : null,
              hören: cfg.hören ? { audioUrl: audioUrl.trim(), teils: cfg.hören.teils.map((t, i) => ({ name: t.name, type: t.type, questions: teilAnswers.hören[i].filter((q) => q.question.trim()) })) } : null,
