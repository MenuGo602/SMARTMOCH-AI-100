...
const ENDPOINTS = {
  ...
  generateExam: "/grading/generate-exam",
  uploadAudio: "/uploads/audio",
};
...
export const api = {
  ...
  // ---- AI imtihon generatsiyasi (Hörendan tashqari barcha bo'limlar) ----
  generateExam: (level, lesenTeilTypes, sprachbausteineTeilTypes, sprechenCount) =>
    request(ENDPOINTS.generateExam, {
      method: "POST",
      body: { level, lesenTeilTypes, sprachbausteineTeilTypes, sprechenCount },
    }),

  // ---- Audio fayl yuklash (Hören uchun, telefondan to'g'ridan-to'g'ri) ----
  // Backend nisbiy yo'l ("/files/...") qaytaradi, shu yerda to'liq URL'ga aylantiramiz,
  // chunki <audio src> butun domen bilan ishlashi kerak.
  uploadAudio: async (file) => {
    const form = new FormData();
    form.append("file", file);
    const res = await fetch(`${BASE_URL}${ENDPOINTS.uploadAudio}`, {
      method: "POST",
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new ApiError(text || `Yuklashda xatolik: ${res.status}`, res.status);
    }
    const data = await res.json();
    // BASE_URL odatda ".../api/v1" bilan tugaydi — /files/ undan tashqarida (backend root'ida).
    const origin = BASE_URL.replace(/\/api\/v1\/?$/, "");
    return `${origin}${data.url}`;
  },

  // ---- O'qituvchi bo'lish uchun to'lov so'rovi (chek rasmi bilan) ----
  requestTeacher: async (receiptFile) => {
    const form = new FormData();
    form.append("receipt", receiptFile);
    // Content-Type ni qo'lda belgilamaslik kerak — browser FormData uchun
    // "multipart/form-data; boundary=..." ni o'zi to'g'ri qo'yadi.
    // Agar Content-Type qo'lda belgilansa, boundary yo'qoladi va 400 xato chiqadi.
    const res = await fetch(`${BASE_URL}/payments/request-teacher`, {
      method: "POST",
      headers: { Authorization: `tma ${getInitData()}` },
      body: form,
    });
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new ApiError(text || `Xatolik: ${res.status}`, res.status);
    }
    return res.json();
  },
};
